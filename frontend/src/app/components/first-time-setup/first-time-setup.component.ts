  ...
  handleUploadImage() {
    ...
    if (typeof (FileReader) !== 'undefined') {
      ...
      reader.onload = (e: any) => {
        ...
        formData.append('avatar', new Blob([e.target.result], {type:inputNode.files[0].type}), inputNode.files[0].name);
        ...
        this.userService.changeAvatar(formData).subscribe(
          ...
          err => {
            this.success = false;
            this.errorMessage = err.error.statusCode + ": " +err.error.message;
          }
