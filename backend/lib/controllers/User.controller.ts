...
export const readUserByUsername = async (req: Request, res: Response) => {
  ...
  if (!r || r.get("u") == null)
    throw new ErrorHandler(HTTP.NotFound, "No such user exists");
  ...
  if (!result.records.length)
    next(
      new ErrorHandler(HTTP.NotFound, [
        { param: "email", msg: "No such user for this email" },
      ])
    );
...
export const logoutUser = (req: Request, res: Response, next: NextFunction) => {
  if (req.session) {
    req.session.destroy((err) => {
      if (err)
        return next(new ErrorHandler(HTTP.ServerError, "Logging out failed"));
      res.status(HTTP.OK).end();
    });
  }
};
...
export const readFollowers = async (req: Request, res: Response) => {
  ...
  let followers = result.records.map((record: any) =>
    filterUserFields(record.get("follower").properties)
  );
  res.json(followers);
};
...
export const readBlockedUsers = async (req: Request, res: Response) => {
  ...
  let blockees = result.records.map((record: any) =>
