...
export const getN4jNodeName = (postable_type:string):string => {
    switch(postable_type) {
        case PostableType.Plant:     return 'Plant';
        case PostableType.Garden:    return 'Garden';
        case PostableType.Device:    return 'Device';
        case PostableType.Post:      return 'Post';
    }
}
...
export const unheartPostable = async (req:Request, res:Response) => {
    await cypher(`
        MATCH (u:User {_id:$uid})-[h:HEARTS]->(o:${getN4jNodeName(res.locals.type)} {_id:$oid})
        WHERE o:Plant OR o:Garden OR o:Device OR o:Post
        DELETE h
    `, {
        oid: req.params.rid ?? req.params.did ?? req.params.pid,
