  ...
  describe("test user routes", () => {
    ...
    describe("run the tests", () => {
      it("should verify the user email", (done) => {
        request(T.app)
          .get(`/auth/verify?email=${T.get("USER").email}&hash=nothing`)
          .end((err, res) => {
      ...
      it("should verify the secondary user email", (done) => {
        request(T.app)
          .get(`/auth/verify?email=${T.get("USER", true).email}&hash=nothing`)
          .expect(301)
          .end((err, res) => {
            done(err);
          });
      ...
      it("should get user by username", (done) => {
        request(T.app)
          .get(`/users/u/${T.get("USER").username}`)
          .expect(200)
          .end((err, res) => {
            expect(res.body._id).to.be.eq(T.get("USER")._id);
            done(err);
      ...
      it("should update the users avatar", (done) => {
        request(T.app)
          .put(`/users/${T.get("USER")._id}/avatar`)
      ...
      it("should follow the other user", (done) => {
        request(T.app)
          .post(`/users/${T.get("USER")._id}/follow/${T.get("USER", true)._id}`)
          .expect(200)
          .end((err, res) => {
            done(err);
          });
      ...
      it("should get a list of followers", (done) => {
        request(T.app)
          .get(`/users/${T.get("USER", true)._id}/followers`)
          .expect(200)
          .end((err, res) => {
      ...
      it("should un-follow the other user", (done) => {
        request(T.app)
          .delete(
            `/users/${T.get("USER")._id}/follow/${T.get("USER", true)._id}`
          )
          .expect(200)
          .end((err, res) => {
            done(err);
      ...
      it("should block the other user", (done) => {
        request(T.app)
          .post(`/users/${T.get("USER")._id}/block/${T.get("USER", true)._id}`)
          .expect(200)
          .end((err, res) => {
            done(err);
          });
      ...
      it("should get a list of blocked users", (done) => {
        request(T.app)
          .get(`/users/${T.get("USER")._id}/blocking`)
          .expect(200)
          .end((err, res) => {
            expect(res.body.length).to.be.eq(1);
            done(err);
      ...
      it("should un-block the other user", (done) => {
        request(T.app)
          .delete(
            `/users/${T.get("USER")._id}/block/${T.get("USER", true)._id}`
          )
          .expect(200)
          .end((err, res) => {
            done(err);
      ...
      it("should delete the secondary user", (done) => {
        request(T.app)
          .delete(`/users/${T.get("USER", true)._id}`)
          .expect(200)
