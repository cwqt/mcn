  ...
  describe("test posting", () => {
    ...
    describe("run tests on data", () => {
      it("should get both posts from the non-alt user", (done) => {
        request(T.app)
          .get(`/users/${T.get("USER")._id}/posts`)
          .end((err, res) => {
      ...
      it("should make alt user reply to user post", (done) => {
        request(T.app)
          .post(
            `/users/${T.get("USER", true)._id}/posts/${T.get("POST")._id}/reply`
          )
      ...
      it("should make alt user reply to users reply", (done) => {
        request(T.app)
          .post(
            `/users/${T.get("USER", true)._id}/posts/${
              T.get("REPLY")._id
            }/reply`
          )
          .send({ content: "this is a reply" })
      ...
      it("should make user heart alt users post", (done) => {
        request(T.app)
          .post(
            `/users/${T.get("USER")._id}/posts/${T.get("POST", true)._id}/heart`
          )
          .set("Content-Type", "application/json")
          .expect(201)
          .end((err, res) => {
      ...
      it("should get the alt users post & it have a heart", (done) => {
        request(T.app)
          .get(
            `/users/${T.get("USER", true)._id}/posts/${T.get("POST", true)._id}`
          )
          .expect(200)
          .end((err, res) => {
